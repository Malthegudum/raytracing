...
def createScene():
	...
	ground = [0, 0.2, np.asarray(Image.open("C:/Users/Bruger/Documents/python/opencv/3dshit/assets/skak.jpg")), 1]
	sky = [np.asarray(Image.open("C:/Users/Bruger/Documents/python/opencv/3dshit/assets/skypan.jpg")), np.asarray(Image.open("C:/Users/Bruger/Documents/python/opencv/3dshit/assets/skytop.jpg"))]
	...
	for i in range(nspheres):
		...
		shine = random.random()
		sphere.append([r, center, colour, shine])
...
def intersection(sphere, ground, sky, light, rv, campos, bounces):
	...
	if tmin != None:
		...
		# Fining the direction vector for the new ray
		nv = pos - sphere[imin][1]
		rv2 = np.dot(-2 * np.dot(rv, nv) / np.linalg.norm(nv), nv) + rv

		# Nudging the intersecting position away from the object so it wont intersect with it self
...
if __name__ == "__main__":
	...
	if animate:
		for i in range(10):
			...
			im.save(f"C:/Users/Bruger/Documents/python/opencv/3dshit/animation/qwerty{i}.jpg")
	else:
		img = raytrace(sphere, ground, sky, light, resolution, lens, campos, camdirection, bounces)
	plt.imshow(img)
	plt.show()